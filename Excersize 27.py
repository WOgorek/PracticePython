...
# So we are going to approximate this clicking simply by asking the user for a coordinate
...
# but if a player tries to put a piece in a game position where there already is another piece,
...
# In a bonus version, keep track of how many squares are full and automatically stop asking
...
def ruch_gracza(gracz, wiersz, kolumna, gra):
    ...
    for i in range(3):
        print("{} {} {}".format(gra[i][0], gra[i][1], gra[i][2]))
    print()
    return gra
...
while True:
    ...
    #print("Wiersz = {} , Kolumna = {}".format(wiersz, kolumna))

# game = ruch_gracza(1, 2, 2, game)
# game = ruch_gracza(2, 1, 1, game)
# game = ruch_gracza(1, 1, 3, game)
# game = ruch_gracza(2, 1, 3, game)
